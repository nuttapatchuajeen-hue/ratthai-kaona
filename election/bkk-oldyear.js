...
(function () {
  ...
  function render() {
    ...
    el('sumView').innerHTML = ''
      ...
      + '    <div class="sumwin" style="background:linear-gradient(135deg,' + col + ',' + shade(col) + ')">'
      ...
      + '    </div>'
      + '    <div class="oybars">'
      + '      <div class="oybar"><span class="oyk">ผู้ชนะ</span><span class="oyt"><i style="width:' + (O.votes / maxV * 100).toFixed(1) + '%;background:' + col + '"></i></span><span class="oyn">' + fmt(O.votes) + '</span></div>'
      + '      <div class="oybar"><span class="oyk">อันดับ 2</span><span class="oyt"><i style="width:' + (O.runV ? (O.runV / maxV * 100).toFixed(1) : 0) + '%;background:var(--line-2)"></i></span><span class="oyn">' + fmt(O.runV) + '</span></div>'
      + '    </div>'
      + '    <ul class="sumfacts">'
      + '      <li><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 6 9 17l-5-5"/></svg><span>อันดับ 2 <b>' + O.run + '</b>' + (O.runP ? ' · ' + O.runP : '') + (O.runV ? ' · ' + fmt(O.runV) + ' คะแนน' : '') + '</span></li>'
